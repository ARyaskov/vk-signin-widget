...
function ensureParamsAreGood(props: VKButtonProps) {
  if (!props.appId) {
    throw new Error("VKButton: appId is required")
  }
  if (!props.redirectURL) {
    throw new Error("VKButton: redirectURL is required")
  }
}
...
export function VkSignInButton(props: VKButtonProps): JSX.Element {
  ...
  const onClickHandler = (e: any) => {
    ...
    if (props.onClickBeforeLogin) props.onClickBeforeLogin(e)
    VKSDK.Config.set({
      app: props.appId,
      redirectUrl: props.redirectURL,
      state: props.state,
      mode: props.loginPageParams?.mode,
    })
